...
const Productos = () => {
  ...
  const products = [
    ...
    { 
      ...
      category: 'Hortalizas y verduras' 
    }
  ];
  ...
  const sendMessage = () => {
    ...
    setTimeout(() => {
      const botResponse = { 
        text: "🤖 Estoy procesando tu consulta sobre productos agrícolas. ¿Necesitas información específica sobre algún producto?", 
        isUser: false 
      };
    ...
    <div className="bg-gray-100 text-gray-800 font-sans">
      <header className="bg-white shadow-sm sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 py-6 flex flex-col md:flex-row justify-between items-center text-white">
          ...
          <nav className="flex gap-6 mt-4 md:mt-0">
            ...
            <Link className="text-black text-lg border-b-2 border-green-700 hover:text-green-700 font-bold transition-colors" to="/productos">Productos</Link>
        ...
        <div className="border-b border-gray-200 bg-white">
          <div className="max-w-3xl mx-auto flex">
            <Link to="/productos" className="w-1/2 py-3 text-center font-semibold border-b-2 border-green-700 text-green-700">Productos</Link>
            <Link to="/ofertas" className="w-1/2 py-3 text-center font-semibold border-b-2 border-transparent text-gray-500 hover:text-green-700">Ofertas</Link>
      ...
      >
        <section className="max-w-7xl mx-auto px-4 py-10 bg-white rounded-lg shadow-md">
          <h2 className="text-3xl font-bold text-center mb-10 text-green-800">Nuestros Productos</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {products.map((product) => (
              <div 
                key={product.id}
                className="product-card cursor-pointer border rounded-xl p-4 shadow-md hover:shadow-lg bg-white transition-all duration-300"
      ...
      {isModalOpen && selectedProduct && (
        ...
        >
          <div 
            ref={modalContentRef}
            className="bg-white rounded-2xl shadow-2xl w-full max-w-xl transform scale-95 opacity-0 transition-all duration-300 p-6 relative flex flex-col gap-6"
          ...
          >
            <button 
              onClick={closeModal}
              className="absolute top-4 right-4 text-gray-500 hover:text-gray-800 transition text-3xl font-light"
            >
              &times;
            </button>
            ...
            <div className="flex flex-col md:flex-row gap-6">
              ...
              <div className="md:w-1/2 flex flex-col justify-between">
                <div>
                  <h3 className="text-2xl font-bold mb-2 text-green-800">{selectedProduct.title}</h3>
                  <p className="text-gray-600 mb-3">{selectedProduct.description}</p>
                  <p className="text-green-700 font-bold text-xl mb-4">{selectedProduct.price}</p>
                  
                  <div className="text-gray-700 mb-2">
                    <span className="font-semibold">Proveedor:</span> 
                    <Link 
                      to={`/business/perfil?proveedor=${encodeURIComponent(selectedProduct.provider)}`}
                  ...
                  <div className="text-gray-700">
                    ...
                    <span className="text-green-700 font-bold ml-1">{selectedProduct.category}</span>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 mt-6">
                  ...
                  <a 
                    href={`https://wa.me/50588888888?text=Hola%20${encodeURIComponent(selectedProduct.provider)},%20me%20interesa%20tu%20producto%20${encodeURIComponent(selectedProduct.title)}`}
                    target="_blank"
